...
PathingTestData::Outcome PathingTestData::operator==(const PathingTestData &rhs)
{
    ...
    if (requiresVisualConfirmation == false)
    {
        if (hasSolution == true)
        {
            const bool equivalentPath = std::fabs((distCard + distDiag * std::sqrtf(2.0f)) - (rhs.distCard + rhs.distDiag * std::sqrtf(2.0f))) < 0.001f;
...
PathingTestData::Outcome PathingTestData::Compare(const PathingTestData& rhs)
{
    return operator==(rhs);
}
